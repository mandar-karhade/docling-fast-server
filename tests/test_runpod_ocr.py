#!/usr/bin/env python3
"""
Test RunPod OCR Endpoint
========================
Test the /ocr endpoint on RunPod deployment.
"""
...
def test_runpod_ocr_with_pdf():
    """Test the OCR endpoint on RunPod with a real PDF file"""
    ...
    print(f"📄 Testing RunPod OCR with PDF: {pdf_path.name} ({pdf_path.stat().st_size / 1024:.1f} KB)")
        ...
        if response.status_code == 200:
            ...
            for file_type, content in files.items():
                if file_type == 'converted_doc':
                    ...
                    output_file = Path(f"output/runpod_{pdf_path.stem}.{file_type}")
    ...
    print("\n🎉 RunPod OCR test completed!")

if __name__ == "__main__":
    test_runpod_ocr_with_pdf()
